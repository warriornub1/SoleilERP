...
namespace SERP.Infrastructure.Transactions.InboundShipments
{
    public class InboundShipmentRequestRepository : GenericRepository<InboundShipmentRequest>, IInboundShipmentRequestRepository
    {
        ...
        public IQueryable<PagedIsrResponseDetail> BuildISRFilterQuery(PageFilterIsrRequestModel request)
        {
            ...
            if (request.cargo_ready_date_from.HasValue)
            {
                isrQuery = isrQuery.Where(x => x.cargo_ready_date >= request.cargo_ready_date_from);
            }

            if (request.cargo_ready_date_to.HasValue)
            {
                isrQuery = isrQuery.Where(x => x.cargo_ready_date <= request.cargo_ready_date_to);
            }
            ...
            var asnQuery = from asnHeader in _dbContext.AsnHeaders.AsNoTracking()
                ...
                select new InboundShipmentRequestPageAsnDetail
                {
                    id = asnHeader.id,
                    ...
                    invoice = (from invoiceHeader in _dbContext.InvoiceHeaders.AsNoTracking()
                        join currency in _dbContext.Currency.AsNoTracking() on invoiceHeader.currency_id equals currency.id
                        where invoiceHeader.asn_header_id == asnHeader.id
            ...
            var query = from isr in isrQuery
                        ...
                        select new PagedIsrResponseDetail
                        {
                            ...
                            asn = asnQuery.FirstOrDefault(x => x.id == isr.asn_header_id)
                        };
