...
namespace SERP.Infrastructure.Transactions.Containers
{
    internal class ContainerASNRepository : GenericRepository<ContainerASN>, IContainerAsnRepository
    {
        ...
        public async Task<bool> ValidateShipmentTypeInAsnAsync(int containerId, int asnHeaderId, string shipmentType)
        {
            ...
            // return false: if this asn already mapping with container has "Loose" shipment type
            // return true: if this asn does not have mapping with container has "Loose" shipment type
            return await existingContainerShipmentType.CountAsync(x => x.shipment_type.Equals(DomainConstant.Containers.ShipmentType.Loose)) == 0;
        }
    }
}
