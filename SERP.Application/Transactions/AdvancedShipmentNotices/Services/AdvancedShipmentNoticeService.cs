...
namespace SERP.Application.Transactions.AdvancedShipmentNotices.Services
{
    internal class AdvancedShipmentNoticeService : IAdvancedShipmentNoticeService
    {
        ...
        public async Task<int[]> CreateAsnAsync(string userId, CreateASNRequestDto request)
        {
                        ...
                        case DomainConstant.AdvancedShipmentNotices.BlAwbProvider.No:
                            {
                                ...
                                // - if new_inbound_shipment_group_flag is true then generate a new inbound_shipment_request_group_no in InboundShipmentRequest
                                if (asn.shipping_information.new_shipment_group_flag)
                                {
                                    var seqIsg =
                                        await _sequenceTrackingRepository.GetSequenceNoByType(SequenceTracking.Type
                                            .InboundShipmentRequestGroup);
                                ...
                                {
                                    inboundShipmentRequest.inbound_shipment_request_group_no = asn.shipping_information.inbound_shipment_request_group_no;
                                }
            ...
            try
            {
                ...
                foreach (var asnInfo in asnInfos)
                {
                    ...
                    #region InboundShipmentRequest
                    if (asnInfo.InboundShipmentRequest is not null)
                    {
                        asnInfo.InboundShipmentRequest.asn_header_id = asnInfo.Header.id;
                        inboundShipmentRequestToInsert.Add(asnInfo.InboundShipmentRequest);
                    }
                    #endregion
                }
                ...
                if (containerToInsert.Count > 0)
                {
                    ...
                    if (containerAsnToInsert.Count > 0)
                    {
                        ...
                        var packingHeaderToInsert = containerAsnToInsert.Select(x => new PackingHeader
                        {
                            ...
                            packing_list_no = x.packing_list_no,
                            created_by = userId,
                            created_on = DateTime.Now
                        }).ToList();
        ...
        {
            var packingLIstInformation = await _packingHeaderRepository.GetPackingHeaderAndContainerInfoAsync(x => x.asn_header_id == filter.asn_header_id && x.container_id == filter.container_id);

            if (packingLIstInformation.Count == 0)
            {
            ...
            {
                PackingHeaderIds = packingLIstInformation.Select(x => x.packing_header_id).ToList(),
                Keyword = request.Keyword,
            ...
            if (totalRows == 0)
            {
                return new PagedResponse<ResponsePackingInformationResponseDto>()
                {
                    Items = packingLIstInformation.Select(x => new ResponsePackingInformationResponseDto()
                    {
                        packing_header_id = x.packing_header_id,
                        packing_list_no = x.packing_list_no,
                        seal_no = x.seal_no,
                        container_no = x.container_no,
                        container_type = x.container_type,
                        shipment_type = x.shipment_type,
                        no_of_packages = x.no_of_packages,
                        weight = x.weight,
                        details = []
                    }).ToList()
                };
            ...
            foreach (var item in packingLIstInformation)
            {
                var packingDetails = pagedResponse.Where(x => x.packing_header_id == item.packing_header_id).ToList();
                var packingDetailsResult = new ResponsePackingInformationResponseDto
                {
                    packing_header_id = item.packing_header_id,
        ...
        public async Task<object> AddContainerForAsnAsync(string userId, int asnHeaderId, CreateContainerForAsnRequestDto request)
        {
            ...
            try
            {
                ...
                packingHeader = new PackingHeader
                {
                    ...
                    asn_header_id = asnHeaderId,
                    container_id = container.id,
                    packing_list_no = containerAsn.packing_list_no
                };
        ...
        public async Task UpdateContainerForAsnAsync(string userId, int asnHeaderId, UpdateContainerForAsnRequestDto request)
        {
            ...
            container.seal_no = request.seal_no;
                ...
                case DomainConstant.AdvancedShipmentNotices.BlAwbProvider.No:
                    {
                        ...
                        // - if new_inbound_shipment_group_flag is true then generate a new inbound_shipment_request_group_no in InboundShipmentRequest
                        if (request.shipping_information.new_shipment_group_flag)
                        {
                            var seqIsg =
                                await _sequenceTrackingRepository.GetSequenceNoByType(SequenceTracking.Type
                                    .InboundShipmentRequestGroup);
                        ...
                        {
                            inboundShipmentRequest.inbound_shipment_request_group_no = request.shipping_information.inbound_shipment_request_group_no;
                        }
        ...
        private async Task<AsnDetailResponseDto> GetAdvanceShipmentNoticeInfo(ASNHeader asnHeader)
        {
            ...
            result.containers = _mapper.Map<List<ContainerResponse>>(await _containerRepository.GetContainerListForAsnAsync(asnHeader.id));
