...
namespace SERP.Application.Common.BackendSchedulerTask.Services
{
    public class CronJobService : ICronJobService, IHostedService, IDisposable
    {
        ...
        public CronJobService(IOptions<CronJobSettings> settings, IServiceScopeFactory scopeFactory)
        {
        ...
        private async void DoWork(object state)
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                try
                {
                    var repository = scope.ServiceProvider.GetRequiredService<ICurrencyService>();
                    await Task.Run(() => repository.UpdateAllBasedCurrencyExchangeFixerIO());
