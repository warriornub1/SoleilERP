...
using SERP.Application.Finance.CostCenters.DTOs.Validator;
...
using System.Text;
using static SERP.Domain.Common.Constants.DomainDBLength;
using CostCenter = SERP.Domain.Finance.CostCenters.CostCenter;
...
namespace SERP.Application.Finance.CostCenters.Services
{
    public class CostCenterService : ICostCenterService
    {
        ...
        public CostCenterService(IUnitOfWork unitOfWork, ICostCenterRepository costCenterRepository, IMapper mapper,
            IGroupRepository groupRepository, ICompanyStructureRepository companyStructureRepository) 
        {
        ...
        public async Task<PagedResponse<CostCenterResponseModel>> SearchPagedAsync(int page, int pageSize, string keyword, SearchCostCenterRequestModel request)
        {
            ...
            var sortedCostCenters = await _costCenterRepository.CostCenterFilterPaged(request.create_date_from, request.create_date_to, groupList, statusList, keyword);
        ...
        private async Task<List<CostCenterTreeResponseModel>> GetSubCostCenter(int parentId, int level)
        {
            ...
            foreach(var group in groups)
            {
                ...
                if(group.level == 1)
                {
                    var costCenters = await _costCenterRepository.GetCostCenterViaParentID(group.id);
                    ...
                    foreach(var costCenter in costCenters)
                    {
                        costCenterLists.Add(new CostCenterList
                        {
                            ...
                            cost_center_description = costCenter.cost_center_description,
        ...
        public async Task<CostCenterResponseModel> GetByIdAsync(int id)
        {
            try
            {
                var costCenter = await _costCenterRepository.GetByIdAsync(x => x.id == id);
        ...
        public async Task CreateCostCenterAsync(CreateCostCenterRequestModel request, string userId)
        {
            try
            {
                ...
                var validator = new CreateCostCenterModelValidator(_companyStructureRepository, _costCenterRepository, _groupRepository);
                ...
                CostCenter cc = new CostCenter
                {
                    ...
                    company_structure_id = request.company_structure_id,
                    created_by = userId,
        ...
        public async Task UpdateCostCenterAsync(List<UpdateCostCenterRequestModel> requests, string userId)
        {
            try
            {
                var validator = new UpdateCostCenterModelValidator();
                ...
                var company_structure_ids = requests.Where(x => x.company_structure_id != null)
                                                    .Select(x => x.company_structure_id.Value)
                                                    .ToList();

                if(company_structure_ids.Count > 0)
                {
                    var companyStructureList = await _companyStructureRepository.GetCompanyStructureCodes(company_structure_ids);
                    var notFoundId = company_structure_ids.Except(companyStructureList);
                    if (notFoundId.Any())
                        throw new BadRequestException(ErrorCodes.CompanyStructureIdNotFound, string.Format(ErrorMessages.CompanyStructureIdNotFound, string.Join(", ", notFoundId)));
                ...
                foreach (var request in requests)
                {
                    ...
                    revenueCenter.company_structure_id = request.company_structure_id;
        ...
        public async Task<List<CCCompanyStructureResponseModel>> GetCompanyStructureCostCenter()
        {
            try
            {
                var result = await _costCenterRepository.FindCompanyStructureCC();

                if (result == null)
                    throw new BadRequestException(ErrorCodes.CompanyStructureRevenueCenterNotFound, ErrorMessages.CompanyStructureRevenueCenterNotFound);


                var response = _mapper.Map<List<CCCompanyStructureResponseModel>>(result);
        ...
        public async Task UpdateCostCenterCompanyStructureAsync(List<UpdateCostCenterCompanyStructureModel> requests)
        {
            try
            {
                _unitOfWork.BeginTransaction();
                var validator = new UpdateCostCenterCompanyStructureValidator();
                ...
                var duplicated_Code = requests.GroupBy(x => x.cost_center_id)
                                               .Where(x => x.Count() > 1)
                                               .Select(x => x.Key)
                                               .ToList();

                if (duplicated_Code.Count() > 0)
                    throw new BadRequestException(ErrorCodes.RevenueCenterIdDuplicated, string.Format(ErrorMessages.DuplicatedValues, "Cost Center Id", string.Join(", ", duplicated_Code)));


                // Check for duplicated company_structure_id
                var company_structure_id = requests.Where(x => x.company_structure_id != null)
                             .Select(x => x.company_structure_id)
                             .ToList();

                var duplicated_company_structure_ido = company_structure_id.GroupBy(x => x)
                                                                           .Where(x => x.Count() > 1)
                                                                           .Select(x => x.Key)
                                                                           .ToList();

                if (duplicated_company_structure_ido.Count() > 0)
                    throw new BadRequestException(ErrorCodes.DuplicatedCompanyId, string.Format(ErrorMessages.DuplicatedValues, "Company Structure id", string.Join(", ", duplicated_company_structure_ido)));


                var update_list_id = requests.Where(x => x.action_flag == ApplicationConstant.ActionFlag.Create)
                                             .Select(x => x.company_structure_id.Value).ToList();

                var update_list = requests.Where(x => x.action_flag == ApplicationConstant.ActionFlag.Create).ToList();
                var delete_list = requests.Where(x => x.action_flag == ApplicationConstant.ActionFlag.Delete).Select(x => x.company_structure_id).ToList();
                var request_id = requests.Select(y => y.cost_center_id).ToList();


                // Get all Ids that are not found
                var revenue_center_dic = await _costCenterRepository.GetDictionaryViaId(request_id);
                var notFoundIds = request_id.Except(revenue_center_dic.Keys.ToList());
                if (notFoundIds.Count() > 0)
                {
                    throw new BadRequestException(ErrorCodes.RevenueCenterIdNotFound, string.Format(ErrorMessages.RevenueCenterIdNotFound, string.Join(", ", notFoundIds)));
                }

                // Get all company_structure_id not found
                var company_structure_ids = await _companyStructureRepository.FindMultiple(x => update_list_id.Contains(x.id) && x.status_flag == DomainConstant.StatusFlag.Enabled);
                var notFoundIdsCS = update_list_id.Except(company_structure_ids.Select(x => x.id).ToList());
                if (notFoundIdsCS.Count() > 0)
                {
                    throw new BadRequestException(ErrorCodes.CompanyStructureIdNotFoundRC, string.Format(ErrorMessages.CompanyStructureIdNotFoundRC, string.Join(", ", notFoundIdsCS)));
                }

                // Check if Organization number is occupy by another dataset
                //var duplicatedCompanyStructureId = await _costCenterRepository.FindMultiple(x => update_list_id.Contains(x.company_structure_id.Value));

                //HashSet<int> duplicated_company_structure = new HashSet<int>();
                //if (duplicatedCompanyStructureId.Count() > 0)
                //{
                //    foreach (var rc in duplicatedCompanyStructureId)
                //    {
                //        // go into request list to check if the company_struct_id is changed
                //        var request = requests.Where(x => x.cost_center_id == rc.id && x.action_flag == ApplicationConstant.ActionFlag.Delete)
                //                              .FirstOrDefault();
                //        // if there are not changes in the request list or if the 
                //        if (request == null || request.company_structure_id != null)
                //            duplicated_company_structure.Add(rc.company_structure_id.Value);

                //    }
                //}
                //if (duplicated_company_structure.Count() > 0)
                //    throw new BadRequestException(ErrorCodes.CompanyStructureOrganizationNoInUse, string.Format(ErrorMessages.CompanyStructureOrganizationNoInUse, string.Join(",", duplicated_company_structure)));

                var duplicatedCompanyStructureId = await _costCenterRepository.FindMultiple(x => update_list_id.Contains(x.company_structure_id.Value));
                if (duplicatedCompanyStructureId.Count() > 0)
                {
                    foreach (var cc in duplicatedCompanyStructureId)
                    {
                        cc.company_structure_id = null;
                    }
                    await _costCenterRepository.UpdateRangeAsync(duplicatedCompanyStructureId);
                    await _unitOfWork.SaveChangesAsync();
                }


                var delete_list_db = new List<CostCenter>();
                foreach (var request in requests.Where(x => x.action_flag == ApplicationConstant.ActionFlag.Delete).Select(x => x.cost_center_id))
                {
                    revenue_center_dic[request].company_structure_id = null;
                    delete_list_db.Add(revenue_center_dic[request]);
                }
                if (delete_list_db.Count() > 0)
                {
                    await _costCenterRepository.UpdateRangeAsync(delete_list_db);
                    await _unitOfWork.SaveChangesAsync();

                    //Reject if company_structure_id is found more than one record in CostCenter table(validate after update> rollback if failed)
                    var costCenterCompanyStructId = (await _costCenterRepository.FindMultiple(x => x.company_structure_id != null)).Select(x => x.company_structure_id).ToList();
                    var duplicatedCompanyStructDBId = costCenterCompanyStructId.GroupBy(x => x)
                                                                             .Where(x => x.Count() > 1)
                                                                             .Select(x => x.Key)
                                                                             .ToList();

                    if (duplicatedCompanyStructDBId.Count > 0)
                    {
                        throw new BadRequestException(ErrorCodes.RevenueCenterCompanyStructDuplicated, string.Format(ErrorMessages.RevenueCenterCompanyStructDuplicated, string.Join(", ", duplicatedCompanyStructDBId.ToList())));
                    }
                }

                var update_list_db = new List<CostCenter>();
                foreach (var request in update_list)
                {
                    revenue_center_dic[request.cost_center_id].company_structure_id = request.company_structure_id;
                    update_list_db.Add(revenue_center_dic[request.cost_center_id]);
                }
                await _costCenterRepository.UpdateRangeAsync(update_list_db);
                await _unitOfWork.SaveChangesAsync();
                _unitOfWork.Commit();
        ...
        public async Task<IEnumerable<CostCenterLimitedModel>> GetAllLimitedAsync(bool onlyEnabled)
        {
            try
            {
                string enabled = onlyEnabled == true ? DomainConstant.StatusFlag.Enabled : DomainConstant.StatusFlag.Disabled;
                var costCenters = await _costCenterRepository.FindMultiple(x => x.status_flag == enabled);

                if (costCenters == null)
                    throw new BadRequestException(ErrorCodes.CostCenterNotFound, ErrorMessages.CostCenterNotFound);

                List<CostCenterLimitedModel> costCenterResponse = _mapper.Map<List<CostCenterLimitedModel>>(costCenters);
                ...
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    while (reader.Read())
                    {
                        if (rowIndex != 0)
                        {
                            ...
                            rowData.organizationNumber = reader.IsDBNull(3) ? null : reader.GetValue(3).ToString();
        ...
        private async Task validateExcelData(string userId, List<CostCenterDto> excelData)
        {
            ...
            try
            {
                ...
                var orgnizationNo = excelData.Where(x => x.organizationNumber != null)
                                                     .Select(x => x.organizationNumber)
                                                     .ToList();

                var getIDByParentGroupNo = await _groupRepository.GetIDByParentGroupNo(parentGroupList, DomainConstant.Group.GroupType.CostCenter);
                var rcDictionary = await _costCenterRepository.GetDictionary(rcCode);
                var companyStructureDictionary = await _companyStructureRepository.GetCompanyOrgNoDic(orgnizationNo);
                ...
                foreach (var data in excelData)
                {
                    ...
                    if (data.organizationNumber != null || !companyStructureDictionary.Keys.Contains(data.organizationNumber))
                ...
                else
                {
                    foreach (var data in excelData)
                    {
                        if (!rcDictionary.Keys.Contains(data.code))
                        {
                            createList.Add(new CostCenter
                            {
                                ...
                                company_structure_id =  data.organizationNumber == null ? null : companyStructureDictionary[data.organizationNumber] ,
                        ...
                        else
                        {
                            ...
                            rcDictionary[data.code].company_structure_id = data.organizationNumber == null ? null : companyStructureDictionary[data.organizationNumber];
                    ...
                    //Reject if company_structure_id is found more than one record in CostCenter table(validate after update> rollback if failed)
                    var costCenterCompanyStructId = ( await _costCenterRepository.GetAllAsync() ).Select(x => x.company_structure_id).ToList();
                    var duplicatedCompanyStructDBId = costCenterCompanyStructId.GroupBy(x => x)
                                                                             .Where(x => x.Count() > 1)
                                                                             .Select(x => x.Key)
                                                                             .ToList();
                    ...
                    if(duplicatedCompanyStructDBId.Count > 0)
                    {
                        throw new BadRequestException( ErrorCodes.CostCenterCompanyStructDuplicated, string.Format(ErrorMessages.CostCenterCompanyStructDuplicated, string.Join( ", ", duplicatedCompanyStructDBId.ToList() )));
                    }
            ...
            */
            try
            {
                ...
                // Check for duplicated code
                var duplicated_Code = excelData.GroupBy(x => x.code)
                                               .Where(x => x.Count() > 1)
                                               .Select(x => x.Key)
                                               .ToList();

                if (duplicated_Code.Count() > 0)
                    throw new BadRequestException(ErrorCodes.DuplicatedCompanyId, string.Format(ErrorMessages.DuplicatedValues, "Company Code", string.Join(", ", duplicated_Code)));


                // Check for duplicated organizationNo
                var organizationNo = excelData.Where(x => x.organizationNumber != null)
                             .Select(x => x.organizationNumber)
                             .ToList();

                var duplicated_org_No = organizationNo.GroupBy(x => x)
                                                     .Where(x => x.Count() > 1)
                                                     .Select(x => x.Key)
                                                     .ToList();

                if (duplicated_org_No.Count() > 0)
                    throw new BadRequestException(ErrorCodes.DuplicatedCompanyId, string.Format(ErrorMessages.DuplicatedValues, "Organization Number", string.Join(", ", duplicated_org_No)));


                // Check If Organization number exist in DB
                ...
                var companyStructureDictionary = await _companyStructureRepository.GetCompanyOrgNoDic(organizationNo);

                foreach (var data in excelData)
                {
                    if (data.organizationNumber != null && !companyStructureDictionary.Keys.Contains(data.organizationNumber))
                        companyError.Add(row + 1);
                    row++;

                }
                if (companyError.Count > 0)
                    sb.Append(string.Format(ErrorMessages.CompanyNotFound, "Organisation Number", string.Join(", ", companyError)));
                ...
                // Check if Organization number is occupy by another dataset
                var duplicatedCompanyStructureId = await _costCenterRepository.FindMultiple(x => companyStructureDictionary.Values.ToList().Contains(x.company_structure_id.Value));

                HashSet<string> duplicated_company_structure = new HashSet<string>();
                if (duplicatedCompanyStructureId.Count() > 0)
                {
                    var companyStructDic = await _companyStructureRepository.GetCompanyOrgIdDic(companyStructureDictionary.Values.ToList());
                    foreach (var rc in duplicatedCompanyStructureId)
                    {
                        var request = excelData.Where(x => x.code == rc.cost_center_code).FirstOrDefault();
                        if (request == null || request.organizationNumber != null && companyStructureDictionary[request.organizationNumber] == rc.company_structure_id)
                            duplicated_company_structure.Add(companyStructDic[rc.company_structure_id.Value]);
                ...
                foreach (var data in excelData)
                {
                    if (data.code is null || data.code == null ||
                        ...
                        data.status is null || data.status == null)
                    {
                        missingDataRow.Add(row + 1);
                    }

                    if (!(data.status == DomainConstant.StatusFlag.Enabled || data.status == DomainConstant.StatusFlag.Disabled))
                    {
                        invalidStatusFlag.Add(row + 1);
                    }

                    // Reject if parent_group_no is not exist in Group table with group_type=RC, status_flag=E
                    if (!getIDByParentGroupNo.Keys.Contains(data.parentGroup))
                        parentGroupError.Add(row + 1);
