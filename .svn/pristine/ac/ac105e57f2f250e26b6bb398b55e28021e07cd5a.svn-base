...
namespace SERP.Application.Transactions.InboundShipments.Services
{
    internal class InboundShipmentService : IInboundShipmentService
    {
        ...
        public async Task<int[]> CreateInboundShipmentAsync(string userId, List<CreateInboundShipmentRequestDto> requests)
        {
            ...
            var inboundShipmentRequestByAsnHeader = new List<InboundShipmentRequest>();
            var asnHeaderId = requests.SelectMany(x => x.asnList).ToList();
            if (asnHeaderId.Count > 0)
            {
                inboundShipmentRequestByAsnHeader =
                    await _inboundShipmentRequestRepository.GetByConditionAsync(x =>
                        asnHeaderId.Contains(x.asn_header_id));

                // - Update InboundShipmentRequest record to status_flag to 10: Arranged by asn_header_id
                foreach (var inboundShipmentRequest in inboundShipmentRequestByAsnHeader)
                {
                    inboundShipmentRequest.status_flag = DomainConstant.InboundShipmentsRequest.StatusFlag.Arranged;
            ...
            try
            {
                ...
                foreach (var ishInfo in ishInfos)
                {
                    if (ishInfo.AsnList is not null)
                    {
                        ...
                        // - Insert into InboundShipmentASN for mapping of InboundShipment to ASNHeader
                        foreach (var asnHeader in asnHeaders)
                        {
                            var inboundShipmentAsn = new InboundShipmentASN
                            {
                            ...
                            inboundShipmentAsnToInsert.Add(inboundShipmentAsn);
                        }
                    }
                ...
                if (inboundShipmentRequestByAsnHeader.Count > 0)
                {
                    await _inboundShipmentRequestRepository.UpdateRangeAsync(inboundShipmentRequestByAsnHeader);
                    _logger.LogInformation($"-Updated inboundShipmentRequest: {inboundShipmentRequestByAsnHeader.Count} rows");
                }

                if (inboundShipmentAsnToInsert.Count > 0)
                {
                    await _inboundShipmentAsnRepository.CreateRangeAsync(inboundShipmentAsnToInsert);
                    _logger.LogInformation($"-Inserted inboundShipmentAsn: {inboundShipmentAsnToInsert.Count} rows");
                }
        ...
        public async Task UpdateInboundShipmentAsync(string userId, List<UpdateInboundShipmentRequestDto> requests)
        {
            await ValidateCreateIShRequest(_mapper.Map<List<InboundShipmentRequestDto>>(requests));
        ...
        public async Task UpdateInboundShipmentBlAwbAsync(string userId, int inboundShipmentId, List<UpdateInboundShipmentBLAWBRequestDto> requests)
        {
            ...
            foreach (var updateRequest in requests)
            {
                ...
                blAwb.inbound_shipment_id = inboundShipmentId;
        ...
        public async Task<int[]> CreateInboundShipmentBlAwbAsync(string userId, int inboundShipmentId, List<InboundShipmentBLAWBRequestDto> requests)
        {
        ...
        public async Task AddAsnAsync(string userId, MappingAsnRequestDto request)
        {
            ...
            var inboundShipmentRequests = await _inboundShipmentRequestRepository.GetByConditionAsync(x => request.asnList.Contains(x.asn_header_id));
            ...
            var inboundShipmentAsnToInsert = new List<InboundShipmentASN>();
            foreach (var asnHeaderId in request.asnList)
            {
                inboundShipmentAsnToInsert.Add(new InboundShipmentASN
                {
                    created_on = DateTime.Now,
                    created_by = userId,
                    inbound_shipment_id = request.inboundShipmentId,
            ...
            try
            {
                ...
                if (inboundShipmentRequests.Count > 0)
                {
                    ...
                    _logger.LogInformation($"AddAsnAsync: Updated inboundShipmentRequest: {inboundShipmentRequests.Count} rows");
                }

                if (inboundShipmentAsnToInsert.Count > 0)
                {
                    await _inboundShipmentAsnRepository.CreateRangeAsync(inboundShipmentAsnToInsert);
                    _logger.LogInformation($"AddAsnAsync: Created inboundShipmentAsn: {inboundShipmentAsnToInsert.Count} rows");
                }
        ...
        public async Task DeleteAsnAsync(string userId, MappingAsnRequestDto request)
        {
            ...
            var inValidAsnHeaderId = request.asnList.Except(asnHeaderValid).ToArray();
            if (inValidAsnHeaderId.Length > 0)
            {
                throw new BadRequestException(ErrorCodes.ValidationError,
                    string.Format(ErrorMessages.MappingAsnInvalidAsnHeaderId, string.Join(", ", inValidAsnHeaderId)));
            ...
            // - Get inboundShipmentRequest record from ASNHeader. Update inboundShipmentRequest.status to 02:New.
            var inboundShipmentRequests =
                await _inboundShipmentRequestRepository.GetByConditionAsync(x => request.asnList.Contains(x.asn_header_id));
            ...
            var inboundShipmentAsnToDelete = await _inboundShipmentAsnRepository.GetByConditionAsync(x => x.inbound_shipment_id == request.inboundShipmentId && request.asnList.Contains(x.asn_header_id));
            ...
            try
            {
                ...
                {
                    await _inboundShipmentRequestRepository.UpdateRangeAsync(inboundShipmentRequests);
                    _logger.LogInformation($"DeleteAsnAsync: Updated inboundShipmentRequest: {inboundShipmentRequests.Count} rows");
                }

                if (inboundShipmentAsnToDelete.Count > 0)
                {
                    await _inboundShipmentAsnRepository.DeleteRangeAsync(inboundShipmentAsnToDelete);
                    _logger.LogInformation($"DeleteAsnAsync: Deleted inboundShipmentAsn: {inboundShipmentAsnToDelete.Count} rows");
                }
                await _unitOfWork.SaveChangesAsync();
                _unitOfWork.Commit();
            }
        ...
        private async Task<List<ASNHeader>> ValidateCreateIShRequest(List<InboundShipmentRequestDto> request)
        {
            ...
            var asnList = request.Where(x => x.asnList is not null && x.asnList.Count > 0).SelectMany(x => x.asnList).Distinct().ToHashSet();
