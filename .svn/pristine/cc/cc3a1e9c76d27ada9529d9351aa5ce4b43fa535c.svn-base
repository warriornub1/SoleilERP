...
namespace SERP.Infrastructure.Masters.Lovs
{
    public class LovRepository : GenericRepository<Lov>, ILovRepository
    {
        ...
        public IQueryable<Lov> LovFilterAsync(FilterLovRequestModel request)
        {
            var query = _dbContext.Lov.AsNoTracking();
            ...
            {
                query = query.Where(x =>
                    x.lov_value.Contains(request.Keyword) ||
                    x.lov_label.Contains(request.Keyword) ||
                    (x.extended_data_1 != null && x.extended_data_1.Contains(request.Keyword)) ||
                    (x.extended_data_2 != null && x.extended_data_2.Contains(request.Keyword))
                );
            }
        ...
        public async Task<HashSet<string>> FindNaturalAccountType(List<string> naturalAccount, string LovTypes)
        {
            return (await _dbContext.Lov.Where(x => naturalAccount.Contains(x.lov_value) && x.lov_type == LovTypes)
