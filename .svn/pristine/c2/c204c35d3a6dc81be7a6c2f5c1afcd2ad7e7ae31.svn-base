...
namespace SERP.Infrastructure.Transactions.PackingLists
{
    internal class PackingHeaderRepository : GenericRepository<PackingHeader>, IPackingHeaderRepository
    {
        ...
        public async Task<List<PackingListInformationDetail>> GetPackingHeaderAndContainerInfoAsync(Expression<Func<PackingHeader, bool>> predicate)
        {
            var packingHeaderQuery = _dbContext.PackingHeaders.AsNoTracking().Where(predicate);

            var res = from container in _dbContext.Containers.AsNoTracking()
                      join packingHeader in packingHeaderQuery on container.id equals packingHeader.container_id
                      select new PackingListInformationDetail
                      {
                          packing_header_id = packingHeader.id,
                          packing_list_no = packingHeader.packing_list_no,
