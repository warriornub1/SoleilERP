...
using SERP.Application.Finance.RevenueCenters.DTOs.Validator;
...
namespace SERP.Application.Finance.RevenueCenters.Services
{
    public class RevenueCenterService : IRevenueCenterService
    {
        ...
        public RevenueCenterService(IUnitOfWork unitOfWork, IRevenueCenterRepository revenueCenterRepository,
            IGroupRepository groupRepository, IMapper mapper, ICompanyStructureRepository companyStructureRepository)
        {
        ...
        public async Task<PagedResponse<RevenueCenterResponseModel>> GetAllPagedAsync(int page, int pageSize)
        {
            try
            {
                ...
                var revenueCenters = await _revenueCenterRepository.GetAllAsync();
        ...
        public async Task<PagedResponse<RevenueCenterResponseModel>> SearchPagedAsync(int page, int pageSize, string keyword, SearchRevenueCenterRequestModel request)
        {
            ...
            var sortedCostCenters = await _revenueCenterRepository.RevenueCenterFilterPaged(request.create_date_from, request.create_date_to, groupList, statusList, keyword);
        ...
        private async Task<List<RevenueCenterTreeResponseModel>> GetSubRevenueCenter(int parentId, int level)
        {
            ...
            foreach (var group in groups)
            {
                ...
                if (group.level == 1)
                {
                    var revenueCenters = await _revenueCenterRepository.GetRevenueCenterViaParentID(group.id);
        ...
        public async Task<RevenueCenterResponseModel> GetByIdAsync(int id)
        {
            try
            {
                var revenueCenter = await _revenueCenterRepository.GetByIdAsync(x => x.id == id);
        ...
        public async Task CreateRevenueCenterAsync(CreateRevenueCenterRequestModel request, string userId)
        {
            try
            {
                ...
                var validator = new CreateRevenueCenterModelValidator(_companyStructureRepository, _revenueCenterRepository, _groupRepository);
        ...
        public async Task UpdateRevenueCenterAsync(List<UpdateRevenueCenterRequestModel> requests, string userId)
        {
            try
            {
                ...
                var validator = new UpdateRevenueCenterModelValidator();
                ...
                var rcCodes = requests.Select(x => x.revenue_center_code).ToList();

                var revenueCenterHashMap = await _revenueCenterRepository.GetDictionary(rcCodes);
        ...
        public async Task<List<RCCompanyStructureResponseModel>> GetCompanyStructureRevenueCenter()
        {
            try
            {
                var result = await _revenueCenterRepository.FindCompanyStructureRC();
                ...
                var response = _mapper.Map<List<RCCompanyStructureResponseModel>>(result);
        ...
        public async Task UpdateRevenueCenterCompanyStructureAsync(List<UpdateRevenueCenterCompanyStructureModel> requests)
        {

            try
            {
                _unitOfWork.BeginTransaction();
                var validator = new UpdateRevenueCenterCompanyStructureValidator();
                ...
                var duplicated_Code = requests.GroupBy(x => x.revenue_center_id)
                                               .Where(x => x.Count() > 1)
                                               .Select(x => x.Key)
                                               .ToList();

                if (duplicated_Code.Count() > 0)
                    throw new BadRequestException(ErrorCodes.RevenueCenterIdDuplicated, string.Format(ErrorMessages.DuplicatedValues, "Revenue Center Id", string.Join(", ", duplicated_Code)));
                ...
                var request_id = requests.Select(y => y.revenue_center_id).ToList();


                // Get all Ids that are not found
                var revenue_center_dic = await _revenueCenterRepository.GetDictionaryViaId(request_id);
                ...
                //var duplicatedCompanyStructureId = await _revenueCenterRepository.FindMultiple(x => update_list_id.Contains(x.company_structure_id.Value));
                ...
                //        var request = requests.Where(x => x.revenue_center_id == rc.id && x.action_flag == ApplicationConstant.ActionFlag.Delete)
                //                              .FirstOrDefault();
                ...
                if(duplicatedCompanyStructureId.Count() > 0)
                {
                    foreach (var rc in duplicatedCompanyStructureId)
                    {
                        rc.company_structure_id = null;
                    }
                    await _revenueCenterRepository.UpdateRangeAsync(duplicatedCompanyStructureId);
                    await _unitOfWork.SaveChangesAsync();

                }

                var delete_list_db = new List<RevenueCenter>();
                foreach (var request in requests.Where(x => x.action_flag == ApplicationConstant.ActionFlag.Delete).Select(x => x.revenue_center_id))
                {
                ...
                {
                    await _revenueCenterRepository.UpdateRangeAsync(delete_list_db);
                    await _unitOfWork.SaveChangesAsync();
                    ...
                    var costCenterCompanyStructId = (await _revenueCenterRepository.FindMultiple(x => x.company_structure_id != null)).Select(x => x.company_structure_id).ToList();
                ...
                var update_list_db = new List<RevenueCenter>();
                foreach (var request in update_list)
                {
                    revenue_center_dic[request.revenue_center_id].company_structure_id = request.company_structure_id;
                    update_list_db.Add(revenue_center_dic[request.revenue_center_id]);
                }
                await _revenueCenterRepository.UpdateRangeAsync(update_list_db);
                await _unitOfWork.SaveChangesAsync();
                _unitOfWork.Commit();
        ...
        public async Task<IEnumerable<RevenueCenterLimitedModel>> GetAllLimitedAsync(bool onlyEnabled)
        {
            try
            {
                string enabled = onlyEnabled == true ? DomainConstant.StatusFlag.Enabled : DomainConstant.StatusFlag.Disabled;
                var revenueCenters = await _revenueCenterRepository.FindMultiple(x => x.status_flag == enabled);

                if (revenueCenters == null)
                    throw new BadRequestException(ErrorCodes.RevenueCenterNotFound, ErrorMessages.RevenueCenterNotFound);

                List<RevenueCenterLimitedModel> revenueCenterResponse = _mapper.Map<List<RevenueCenterLimitedModel>>(revenueCenters);
        ...
        private async Task validateExcelData(string userId, List<RevenueCenterDto> excelData)
        {
            try
            {
                ...
                // Check if Organization number is occupy by another dataset
                var duplicatedCompanyStructureId = await _revenueCenterRepository.FindMultiple(x => companyStructureDictionary.Values.ToList().Contains(x.company_structure_id.Value));
                ...
                if(duplicatedCompanyStructureId.Count() > 0)
                {
                    ...
                    foreach(var rc in duplicatedCompanyStructureId)
                    {
                        var request = excelData.Where(x => x.code == rc.revenue_center_code).FirstOrDefault();
