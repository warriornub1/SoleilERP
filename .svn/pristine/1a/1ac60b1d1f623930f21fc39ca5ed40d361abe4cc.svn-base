...
namespace SERP.Application.Transactions.PurchaseOrders.Services
{
    internal class PurchaseOrderService : IPurchaseOrderService
    {
        ...
        private void CalculateAmountAndDiscount(POHeader poHeader, List<PODetail> poDetails)
        {
            ...
            // Total Line Discount is calculated by Sum of each PO Detail discount which is PODetail.unit_discount * qty 
            // SE-319: Change formula to (PODetail.unit_discount * PODetail.qty)
            var totalLineDiscount = poDetails.Sum(itemDetail =>
            {
                if (itemDetail.qty == 0 ||
                    itemDetail.unit_discount == 0 ||
                    itemDetail.status_flag.Equals(PurchaseOrder.StatusFlag.Cancelled))
                {
                    return 0;
                }

                var lineDiscount = (itemDetail.unit_discount ?? 0) * itemDetail.qty;

                return lineDiscount;
            });

            poHeader.total_line_discount = totalLineDiscount;
