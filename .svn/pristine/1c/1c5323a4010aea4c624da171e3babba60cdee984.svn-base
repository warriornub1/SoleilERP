...
namespace SERP.Api.Controllers
{
    ...
    public class RevenueCenterController : Controller
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [HttpPut]
        public async Task<IActionResult> UpdateRevenueCenterCompanyStructure(List<UpdateRevenueCenterCompanyStructureModel> requests)
        {
            await _revenueCenterService.UpdateRevenueCenterCompanyStructureAsync(requests);
            return Ok();
        }
        ...
        {
            var costCenters = await _revenueCenterService.GetAllLimitedAsync(onlyEnabled);
            return Ok(costCenters);
        }
    }
}
