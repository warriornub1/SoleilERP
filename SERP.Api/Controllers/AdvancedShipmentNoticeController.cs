...
namespace SERP.Api.Controllers
{
    ...
    public class AdvancedShipmentNoticeController : ControllerBase
    {
        ...
        {
            var fileUpload = model.attachments.Select(x => x.File).ToList();
            await _systemKvsService.ValidateFileUploadAsync(fileUpload);
            ...
            foreach (var item in model.attachments)
            {
                ...
                {
                    file = item.File,
                    url_path = FileServerHelper.SaveToResourceFolder(_webHostEnvironment.ContentRootPath, item.File,
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateShipping([FromBody] UpdateShippingInformationRequestDto request)
        {
            await _advancedShipmentNoticeService.UpdateShippingAsync(_httpContextService.GetCurrentUserId(), request);
            return Ok();
        }
    }
}
